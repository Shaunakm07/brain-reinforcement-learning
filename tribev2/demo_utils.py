...
class TribeModel(TribeExperiment):
    ...
    def from_pretrained(
        ...
        print(f"DEBUG: checkpoint_dir before cleaning: {repr(checkpoint_dir)}")
        checkpoint_dir_str = str(checkpoint_dir).replace('\t', '').strip()
        print(f"DEBUG: checkpoint_dir after cleaning: {repr(checkpoint_dir_str)}")
        ...
        if checkpoint_dir_path.exists():
            ...
            config_path = hf_hub_download(repo_id.replace("\\", "/"), "config.yaml")
            ckpt_path = hf_hub_download(repo_id, checkpoint_name)
        with open(config_path, "r") as f:
            yaml_content = f.read()
            # Replace PosixPath with Path for cross-platform compatibility
            yaml_content = yaml_content.replace('pathlib.PosixPath', 'pathlib.Path')
            # Handle PosixPath on Windows
            import pathlib
            if not hasattr(pathlib, 'PosixPath'):
                pathlib.PosixPath = pathlib.Path
            # Add constructor for PosixPath to handle cross-platform paths
            yaml.add_constructor('!python/object:pathlib.PosixPath', lambda loader, node: Path(loader.construct_scalar(node)), Loader=yaml.UnsafeLoader)
            config = ConfDict(yaml.load(yaml_content, Loader=yaml.UnsafeLoader))
